...
def deconv_layers(model, CLASSES = 5):
    ...
    model.add(Deconvolution2D(64, 3, 3,
            ...
            activation="relu"))
    model.add(Cropping2D(cropping=((1, 1), (1, 1))))
    model.add(Deconvolution2D(5, 3, 3,
            output_shape=(None, 5, 226, 226),
            subsample=(1, 1),
            border_mode='valid',
            activation="relu"))
    model.add(Cropping2D(cropping=((1, 1), (1, 1))))
    model.add(Reshape((5, 224*224)))
    model.add(Permute((2, 1)))
    model.add(Activation(("softmax")))
    return model

def DeconvNet(CLASSES = 5):
    model = Sequential()
    return deconv_layers(bridge(conv_layers(model)),5)
