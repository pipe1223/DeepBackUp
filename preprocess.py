...
PATH1 = MAIN_PATH + "JPEGImages/"
PATH2 = MAIN_PATH + "SegmentationClass/"
filename = MAIN_PATH + "ImageSets/Segmentation/train.txt"
...
def resize_img():
    ...
    for imgname in ls:
        ...
        cv2.imwrite(MAIN_PATH + 'train_voc2012_seg_224/{}.jpg'.format(imgname), train_img)
        cv2.imwrite(MAIN_PATH + 'target_voc2012_seg_224/{}.png'.format(imgname), target_img)
...
def make_target_dataset(imgs):
    ...
    for imgname in imgs:
        path = MAIN_PATH + "target_voc2012_seg_224/"
        ...
        for x in range(dim[0]):
            for y in range(dim[1]):
                ...
                if np.allclose(color, background):
                    ...
                    target[x,y] = 0
        np.save(MAIN_PATH + "npy_target_224_voc12/{}.npy".format(imgname),target)
        print("Done {}".format(imgname))

def split_n_data(n):
    ls = [line.rstrip('\n') for line in open(filename,"r").readlines()]
    size = int(math.ceil(len(ls)/n))
    #print size
    #print '--'
    #print len(ls)
